...
fun main() {
    ...
    // N(1 ≤ N ≤ 500,000)
    val n = fs.nextInt()
    require(n in 1..500000) { "1 ≤ N ≤ 500,000" }
    val numCards = HashSet<Int>()
    val answerList = mutableListOf<Int>()
    // N개의 정수
    repeat(n) {
        numCards.add(fs.nextInt())
    }
    // M(1 ≤ M ≤ 500,000)
    val m = fs.nextInt()
    require(m in 1..500000) { "1 ≤ M ≤ 500,000" }
    // M개의 정수
    repeat(m) {
        answerList.add(if(numCards.contains(fs.nextInt())) 1 else 0)
    }
